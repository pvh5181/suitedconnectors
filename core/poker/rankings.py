...
SHORT_CARD_ORDER = ['6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A']



def best_hand_using_holecards_shortdeck(holecards, board, n_holecards=2):
    ...
    return best_hand_shortdeck(possible_hands)


def best_hand_from_cards_shortdeck(cards):
    if len(cards) < 5:
        raise Exception('Need 5 cards to make a poker hand')
    return best_hand_shortdeck(combinations(cards, 5))


def best_hand_shortdeck(hands):
    return sorted(hands, key=hand_sortkey_shortdeck, reverse=True)[0]


def hand_sortkey_shortdeck(hand):
    return handrank_encoding_to_key_shortdeck(handrank_encoding_shortdeck(hand))


def handrank_encoding_to_key_shortdeck(handrank_encoding):
    n = 0
    m = 1
...
def handrank_encoding_to_key(handrank_encoding):
    n = 0
    m = 1
    for k in handrank_encoding_shortdeck[::-1]:
        if k:
            if isinstance(k, int):  # we're at the N part of the encoding
                n += k * m
            elif k:
...
def handrank_encoding_shortdeck(hand):
    ...
    # full house
        ...
        return [5, buckets[0][0], buckets[1][0], None, None, None]
    # three of a kind
    ...
    # one pair
        ...
        nums = sorted([SHORT_CARD_ORDER.index(card.rank) for card in hand])
        straight = max(nums) - min(nums) == 4
        wheel = nums == [0, 1, 2, 3, 8]
...
def handrank_encoding_to_name_shortdeck(handrank_encoding):
    N, K1, K2, K3, K4, K5 = handrank_encoding
    if N == 0:
        return f'High Card {Card.ranknames[K1]}, {Card.ranknames[K2]} kicker'
    elif N == 1:
...
def hand_to_name_shortdeck(hand):
    return handrank_encoding_to_name_shortdeck(handrank_encoding_shortdeck(hand))