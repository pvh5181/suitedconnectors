...
def check_system_invariants(SUITEDCONNECTORS_ENV):
    ...
    assert SUITEDCONNECTORS_ENV in ALLOWED_ENVS, (
        f'SUITEDCONNECTORS_ENV={SUITEDCONNECTORS_ENV} is not one of the allowed values: '
        f'{",".join(ALLOWED_ENVS)}')
    ...
    try:
        ...
        print('Never run suitedconnectors with python -O, asserts are needed in production.')
        raise SystemExit(1)
...
def check_django_invariants():
    ...
    if 'suitedconnectors-prod' in s.HOSTNAME:
        assert s.SUITEDCONNECTORS_ENV == 'PROD', 'suitedconnectors-prod must run in ENV=PROD mode'
        ...
        repo_dir_prod = s.REPO_DIR == '/opt/suitedconnectors' or s.REPO_DIR == '/opt/suitedconnectors.poker'
        assert repo_dir_prod, 'Repo must be in /opt/suitedconnectors on prod'
    ...
    elif 'suitedconnectors-beta' in s.HOSTNAME:
        assert s.SUITEDCONNECTORS_ENV == 'BETA', 'suitedconnectors-beta must run in ENV=BETA mode'
        assert s.DJANGO_USER == 'www-data', 'Django can only be run as user www-data'
        assert not s.DEBUG, 'DEBUG=True is never allowed on prod and beta!'
        assert s.DEFAULT_HTTP_PROTOCOL == 'https', 'https is required on prod servers'
        assert s.TIME_ZONE == 'UTC', 'Prod servers must always be set to UTC timezone'
        assert s.REPO_DIR == '/opt/suitedconnectors', 'Repo must be in /opt/suitedconnectors on prod'
    ...
    for setting_name in s.SECURE_SETTINGS:
        ...
        if s.SUITEDCONNECTORS_ENV in ('PROD', 'BETA'):
        ...
        if s.SUITEDCONNECTORS_ENV == 'PROD':
...
def django_status_line(fancy: bool=False, truncate: bool=False) -> str:
    ...
    # DEV=settings.py 👤 squash  🆔 24781  📅 1523266746  💾 suitedconnectors@localhost
    sections = (
    ...
    status_line = ''.join((
        ...
        f' {icn(0)}../env/{settings.SUITEDCONNECTORS_ENV}.env',
