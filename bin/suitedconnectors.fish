...
set POSTGRES_DB "suitedconnectors"
switch "$HOSTNAME"
    case 'suitedconnectors-prod'
        echo "suitedconnectors cmd should only be run on DEV machines (not PROD or BETA)."
        exit 1
    case 'suitedconnectors-prod-bulk'
        echo "suitedconnectors should only be run on DEV machines (not PROD or BETA)."
        exit 1
    case 'suitedconnectors-beta'
        echo "suitedconnectors should only be run on DEV machines (not PROD or BETA)."
        exit 1
    case 'suitedconnectors-beta-bulk'
        echo "suitedconnectors cmd should only be run on DEV machines (not PROD or BETA)."
        exit 1
    case '*'
        set -x SUITEDCONNECTORS_ENV "DEV"
end
...
function stop --description 'Stop the django development server'
    sudo supervisorctl stop suitedconnectors-django:
    sudo supervisorctl stop suitedconnectors-bg:
...
function shell --description 'Open a django shell_plus'
    activate_venv
    cd "$PY_ROOT"
    env DJANGO_DEBUG_HELPERS=1 PYTHONSTARTUP=$PY_ROOT/suitedconnectors/save_ipython_history.py ./manage.py shell_plus
end
...
function fetchdb --description 'Pull and load the database from a remote [host]'
    ...
    echo "[2/5] Backing up remote db $argv[1]:$POSTGRES_DB to $argv[1]:/opt/suitedconnectors.poker/data/backups/$POSTGRES_DB_<ts>.sql.gz..."
    set dump_file (ssh $argv[1] /opt/suitedconnectors.poker/bin/suitedconnectors-server backup | tail -1 | tr -d '\r')
    
    echo "[3/5] Downloading $argv[1]:/opt/suitedconnectors.poker to localhost:/tank/$argv[1]..."
    mkdir -p /tank/$argv[1]/data/backups
    rsync -r --archive --info=progress2 --exclude=database --exclude=backups $argv[1]:/opt/suitedconnectors.poker/ /tank/$argv[1]
...
function backup --description 'Dump a local db backup to /data/backups'
    ...
    set dump_file "$DATA_DIR/backups/suitedconnectors_"(date +%s)".sql.gz"
    echo $green"[>] Backing up DB $POSTGRES_DB to $normal"(basename $dump_file)"..."
    pg_dump "$POSTGRES_DB" | gzip -9 > "$dump_file"
    echo "$dump_file"
end

function load_backup --description 'Load a [suitedconnectors.sql] backup into the database'
    stop
    echo '[!] Destroying database "suitedconnectors" in 5 sec...'
    sleep 5
    dropdb "$POSTGRES_DB"
    createdb "$POSTGRES_DB"
    psql "$POSTGRES_DB" < $argv[1]
    redis-cli FLUSHALL
    echo "[√] Destroyed database localhost:suitedconnectors and loaded new db from $argv[1]"
end
...
function deploy --description 'Deploy the current branch to beta or prod'
    ...
    deploy_step echo "------------------------------------------------------------------ "
    deploy_step echo $green"[+] Starting deploy process of branch $branch to suitedconnectors-$server ("(date)")...$normal"
    ...
    deploy_step echo "    6. suitedconnectors-server deploy"
    deploy_step echo ""
    ...
    deploy_step echo $green"[6] Deploying $branch to suitedconnectors-$server now..."$normal
    if quiet which brew
        unbuffer ssh -t "suitedconnectors-$server" "suitedconnectors-server deploy $server" 2>&1 | sed -l 's/^/     /' | unbuffer -p tee -a "$DATA_DIR/logs/deploys.log"
    else 
        unbuffer ssh -t "suitedconnectors-$server" "suitedconnectors-server deploy $server" 2>&1 | sed -u 's/^/     /' | unbuffer -p tee -a "$DATA_DIR/logs/deploys.log"
    end
    
    deploy_step git checkout $branch
    deploy_step echo $green"[√] Deployed $branch to origin/$server:$blue suitedconnectors-$server"$normal
end
...
function background --description "Display a process's stdout while it's running in the background"
	bash -c "fish -c 'source \"""$ROOT""\"/bin/suitedconnectors.fish; ""$argv""' &"
end
