...
set REPO_URL "https://github.com/monadical-sas/suitedconnectors.poker"
...
switch "$HOSTNAME"
    ...
    case 'suitedconnectors-beta'
        set -x SUITEDCONNECTORS_ENV "BETA"
    case 'suitedconnectors-beta-bulk'
        set -x SUITEDCONNECTORS_ENV "BETA"
    case 'plum'
        set -x SUITEDCONNECTORS_ENV "PROD"
    case '*'
        echo "suitedconnectors-server cmd should only be run on PROD or BETA servers."
        exit 1
end
...
function stop --description 'Stop all production services'
    ...
    stop_heartbeats
    supervisorctl stop suitedconnectors-django:
    supervisorctl stop suitedconnectors-bg:
end
...
function manage --description 'Run manage.py [cmd] [args] e.g. migrate'
    switch "$argv[1]"
        case 'shell'
            echo 'Use the `suitedconnectors-server shell` command instead.'
            return 1
        case 'shell_plus'
            echo 'Use the `suitedconnectors-server shell` command instead.'
            return 1
...
function shell --description 'Open the production django shell_plus'
    cd "$PY_ROOT"
    if [ "$IS_DOCKER" ]
        bash -c "source .venv/bin/activate; env DJANGO_DEBUG_HELPERS=1 IPYTHONDIR=/data/.ipython CONNECTION_STR='$CONNECTION_STR' PYTHONSTARTUP=$PY_ROOT/suitedconnectors/save_ipython_history.py ./manage.py shell_plus $argv"
    else
        sudo -u $DJANGO_USER bash -c "source .venv/bin/activate; env DJANGO_DEBUG_HELPERS=1 IPYTHONDIR=/data/.ipython CONNECTION_STR='$CONNECTION_STR' PYTHONSTARTUP=$PY_ROOT/suitedconnectors/save_ipython_history.py ./manage.py shell_plus $argv"
    end
end
...
function offsite_backup --description 'Backup and save files to remote [host]'
    ...
    backup
    echo $green"[>] Syncing backup of /opt/suitedconnectors.poker/ to $argv[1]:"$normal"/tank/$HOSTNAME"...
    rsync -r --archive --stats --bwlimit=10000 --exclude=database /opt/suitedconnectors.poker/ $argv[1]:/tank/$HOSTNAME
end
...
function setup_tunnels --description 'Setup stunnels between necessary servers'
    ...
    switch $server
        case 'server'
            ...
            echo "suitedconnectors:$psk" > /etc/stunnel/passwords.txt
            ...
            supervisorctl reread
            supervisorctl update
            supervisorctl restart suitedconnectors-base:postgres-tunnel
            supervisorctl restart suitedconnectors-base:redis-tunnel
...
function source_env --description "Source a bash/posix style .env file"
    # e.g. source_env /opt/suitedconnectors.poker/env/secrets.env
...
function deploy_step --description 'Indent all output by one tab and log to file'
    switch "$argv[1]"
        ...
        case 'manage' 'update' 'migrate' 'start' 'stop' 'clear_caches'
            ...
            bash -c "/opt/suitedconnectors.poker/bin/suitedconnectors-server $args 2>&1 | sed -u 's/^/     /' | unbuffer -p tee -a $DATA_DIR/logs/deploys.log"
        case '*'
